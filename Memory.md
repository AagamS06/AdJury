...
>
> **Last updated:** 2026-09-04 (Day 8 — Submission UI; Week 2 started)
...
**Phase:** 2 (core review engine) — **Week 1 complete (Days 1–7)**; **Week 2 in progress** — Day 8 (submission UI) done, next is the scorecard + JurorCard (Day 9).
...
- [ ] **Week 2 in progress.** Day 8 (submission UI) is done. Next is the scorecard + `JurorCard` (Day 9) — turn the minimal Day 8 result panel into the full Design.md §5 scorecard rendered from the `ReviewResult` the form already receives; then the verdict/score visual system (Day 10), review history (Day 11), and review detail (Day 12). These are well-supported by the Week 1 API and the Day 8 client helpers: `submitReview`/`PERSONA_LABELS` (`src/lib/reviews/review-form.ts`) and the Day 6 read core (`getReviewForCompany` → `PersistedReview`, `listReviewsForCompany` → `ReviewSummary[]`) back the detail page (Day 12) and history list (Day 11); the Day 4 guards gate them. Day 27 wires the company's stored brand guide into the review's `brand_context` (null today).
