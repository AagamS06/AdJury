...
>
> **Last updated:** 2026-09-03 (Day 7 — Week 1 hardening; **Week 1 complete**)
...
**Phase:** 1 (Foundation & personas) — **Week 1 complete (Days 1–7)** → **Phase 2** (core review engine) continues at Week 2 (submission & scorecard UI).
...
- [ ] **Week 1 is complete (Days 1–7).** Continue Phase 2 at **Week 2 — Submission & scorecard UI**, starting with **Day 8** (`/review` submission form calling `POST /api/reviews`), then the scorecard + `JurorCard` (Day 9), verdict/score visual system (Day 10), review history (Day 11), and review detail (Day 12). These UI pages are well-supported by the Week 1 API: `POST /api/reviews` (Day 5) backs the submission form, and the Day 6 read core (`getReviewForCompany` → `PersistedReview`, `listReviewsForCompany` → `ReviewSummary[]`) backs the detail page (Day 12) and the history list (Day 11); the Day 4 guards (`requireSession`/`requireAdmin`) gate them. Day 27 will wire the company's stored brand guide into the review's `brand_context` (null today).
