...
>
> **Last updated:** 2026-09-01 (Day 5 — Review endpoint)
...
- [ ] Continue Phase 2: fetch & list endpoints (Day 6 — `GET /api/reviews/[id]` + a list endpoint, both company-scoped via the session), Week 1 hardening (Day 7), then submission UI, scorecard UI, and review history (Week 2). The Day 5 `createReview` core + `ReviewRequestSchema` and the Day 4 guards (`requireSession`/`requireAdmin`) are ready to back those pages. Day 27 will wire the company's stored brand guide into the review's `brand_context` (null today).
