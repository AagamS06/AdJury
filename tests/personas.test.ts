 ...
 * (anchors, the Day 7 confidence rubric, the Day 21 rewrite guidance, the JSON
 * output contract) that keep the five jurors comparable and their output valid
 * against the schema.
 */
...
describe("each persona system prompt", () => {
  it.each(PERSONAS.map((p) => [p.name, p] as const))(
    "%s embeds its machine-name, anchors, confidence + rewrite guidance, and output contract",
    (name, persona) => {
      ...
      expect(persona.systemPrompt).toContain(REWRITE_GUIDANCE);
  ...
  it("tells the rewrite not to fabricate facts or disclaimers (Day 21 calibration)", () => {
    // The rewrite must stay truthful to the original so a "fix" never adds new
    // compliance risk or invented specifics (Rules.md §3).
    expect(REWRITE_GUIDANCE).toMatch(/never invent/i);
    expect(REWRITE_GUIDANCE).toMatch(/disclaimer/i);
  });
});
